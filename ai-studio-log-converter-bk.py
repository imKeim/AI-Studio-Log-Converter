...
def process_parts(parts: list, md_path: Path) -> list:
    content = []
    for part in parts:
        if 'text' in part:
...
def convert_llm_log_to_markdown(json_path: Path, md_path: Path, config: dict, lang_templates: dict, frontmatter_template: str) -> (bool, str):
    ...
    # Обрабатываем первый ход пользователя + системную инструкцию
    if system_instruction or (chunks and chunks[0].get('role') == 'user'):
        first_turn_content = []
        ...
        if system_instruction:
            ...
            first_turn_content.append(spoiler_block)

        if chunks and chunks[0].get('role') == 'user':
            user_parts = chunks[0].get('parts', [])
            if not user_parts and 'text' in chunks[0]:
                user_parts = [{'text': chunks[0]['text']}]
            
            processed_content = process_parts(user_parts, md_path)
            first_turn_content.extend(processed_content)
            chunks.pop(0)

        if first_turn_content:
            conversation_turns.append(f"{lang_templates['user_header']}\n\n" + "\n\n".join(filter(None, first_turn_content)))

    # Обрабатываем оставшиеся ходы
    pending_thought_blocks = []
    for chunk in chunks:
        role = chunk.get('role')
        turn_content = []
        header = ""
        ...
        if role == 'user':
            header = lang_templates['user_header']
            user_parts = chunk.get('parts', [])
            if not user_parts and 'text' in chunk:
                user_parts = [{'text': chunk['text']}]
            turn_content.extend(process_parts(user_parts, md_path))
        
        elif role == 'model':
            header = lang_templates['model_header']
            if chunk.get('isThought'):
                ...
                if thought_text:
                    ...
                    pending_thought_blocks.append(thought_block)
                continue
            else:
                if pending_thought_blocks:
                    turn_content.extend(pending_thought_blocks)
                    pending_thought_blocks = []
                
                # --- ГЛАВНОЕ ИСПРАВЛЕНИЕ ЗДЕСЬ ---
                # Приоритет отдается полному тексту, если он есть
                complete_text = (chunk.get('text') or '').strip()
                if complete_text:
                    turn_content.append(complete_text)
                else:
                    # Запасной вариант: собираем из частей
                    model_parts = chunk.get('parts', [])
                    turn_content.extend(process_parts(model_parts, md_path))
